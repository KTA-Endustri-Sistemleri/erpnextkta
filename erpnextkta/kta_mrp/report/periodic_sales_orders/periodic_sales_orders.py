...
class SatisAnalizi:
    ...
    def get_exchange_rates(self):
        ...
        rates = frappe.get_all(
            "Currency Exchange",
            filters={"date": exchange_date, "to_currency": "TRY", "for_buying": 1},
    ...
    def append_summary_row(self):
        summary_row = {"tree_key": "<b>GENEL TOPLAM</b>", "indent": 0}
        total = 0
        for _, end in self.periodic_ranges:
            key = scrub(self.get_period_label(end))
    ...
    def get_chart(self):
        ...
        for _, end in self.periodic_ranges:
            label = self.get_period_label(end)
            labels.append(label)
            values.append(summary_row.get(scrub(label), 0))
        return {
            "data": {"labels": labels, "datasets": [{"name": "Satış", "values": values}]},
            "type": "line",
            "colors": ["#1abc9c"]
        }
