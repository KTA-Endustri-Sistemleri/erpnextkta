...
def calculate_machine_capability(doc):
    ...
    study_document = frappe.get_doc(main_doctype, doc)
    upper_limit = study_document.get("upper_limit")
    lower_limit = study_document.get("lower_limit")
    soll_cmk = study_document.get("soll_cmk")
    data = study_document.get(child_table)

    for datum in data:
        list_of_data.append(datum.veri)

    half_point = int(len(list_of_data) / 2)
    for index in range(1, half_point + 1):
        top_half.append(list_of_data[index])
    for index in range(half_point + 1, len(list_of_data) + 1):
        lower_half.append(list_of_data[index])

    sublists = [
        list_of_data[i:i + chunk_size]
        for i in range(0, len(list_of_data), chunk_size)
    ]

    for sublist in sublists:
        xa.append(statistics.mean(sublist))
        si.append(statistics.stdev(sublist))

    # tepe değeri Xq
    xa_max = max(xa)
    # dip değeri  Xq
    xa_min = min(xa)
    xa_diff = xa_max - xa_min

    # standart sapma Si
    standardabw = statistics.mean(si)
    ...
    # tek değerin tepe değeri
    maximum_of_data = max(list_of_data)
    # tek değerin dip değeri
    minimum_of_data = min(list_of_data)
    # ortalama Xqq
    xqq = statistics.mean(list_of_data)
    xq = xqq
    cm = float(0.0)
    if standardabw == 0:
        std_var = soll_standard
    else:
        std_var = standardabw
        cm = toleranzbreite / (6 * standardabw)
        cmk_oben = (upper_limit - xqq) / (3 * standardabw)
        cmk_unten = (xqq - lower_limit) / (3 * standardabw)

        if soll_cmk > cmk_unten or soll_cmk > cmk_oben:
            return "not performed"

    xq_minus_6s = xq - 6 * std_var
    xq_minus_5s = xq - 5 * std_var
    xq_minus_4s = xq - 4 * std_var
    xq_minus_3s = xq - 3 * std_var
    xq_minus_2s = xq - 2 * std_var
    xq_minus_1s = xq - std_var
    xq_plus_1s = xq + std_var
    xq_plus_2s = xq + 2 * std_var
    xq_plus_3s = xq + 3 * std_var
    xq_plus_4s = xq + 4 * std_var
    xq_plus_5s = xq + 5 * std_var
    xq_plus_6s = xq + 6 * std_var

    minus_k = (standardabw / 1000)
    xq_minus_6s_k = xq_minus_6s - minus_k
    xq_minus_5s_k = xq_minus_5s - minus_k
    xq_minus_4s_k = xq_minus_4s - minus_k
    xq_minus_3s_k = xq_minus_3s - minus_k
    xq_minus_1s_k = xq_minus_1s - minus_k
    xq_minus_2s_k = xq_minus_2s - minus_k
    xq_k = xq - minus_k
    xq_plus_1s_k = xq_plus_1s - minus_k
    xq_plus_2s_k = xq_plus_2s - minus_k
    xq_plus_3s_k = xq_plus_3s - minus_k
    xq_plus_4s_k = xq_plus_4s - minus_k
    xq_plus_5s_k = xq_plus_5s - minus_k
    xq_plus_6s_k = xq_plus_6s - minus_k

    standard_deviation_of_data = statistics.stdev(list_of_data)
    spread_of_data = maximum_of_data - minimum_of_data

    oeg = xqq + (2.58 / math.sqrt(5)) * (standardabw / 0.94)
    ueg = xqq - (2.58 / math.sqrt(5)) * (standardabw / 0.94)
    cm_a = xa_max / (6 * standard_deviation_of_data)
    if ueg < xqq:
        return "performed"
    if oeg > xqq:
        return "performed"
    cmk_un = xa_min - lower_limit / (3 * standard_deviation_of_data)
    cmk_ob = upper_limit - xa_max / (3 * standard_deviation_of_data)
    if soll_cmk > cmk_un or soll_cmk > cmk_ob:
        return "not performed"
    return None
