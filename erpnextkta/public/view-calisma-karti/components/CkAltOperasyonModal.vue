  ...
  try {
    ...
    if (props.editData && props.editData.hammadde_tuketimleri) {
       wipPool.value.forEach(w => {
           const match = props.editData.hammadde_tuketimleri.find((r:any) => r.source_wip_ids === w.wip_id);
           if (match && match.hedef_kavite) {
               w.hedef_kavite = match.hedef_kavite;
           }
       });
    }
  } finally {
          ...
          <div class="ck-form-group" style="margin-top: 10px; border-top: 1px dashed var(--ck-glass-border-soft); padding-top: 20px;">
            ...
            <div v-if="wipPool.length > 0" style="border: 1px solid var(--ck-primary); border-radius: 8px; padding: 12px; margin-bottom: 16px; background: rgba(0,0,0,0.02);">
              ...
              <div v-else style="display: flex; flex-direction: column; gap: 8px;">
                <label v-for="wip in wipPool" :key="wip.wip_id" style="display: flex; align-items: center; gap: 8px; cursor: pointer; padding: 4px; border-radius: 4px;" :style="{ background: selectedWipIds.includes(wip.wip_id) ? 'rgba(0,0,0,0.05)' : 'transparent' }">
                  ...
                  <div v-if="selectedWipIds.includes(wip.wip_id)" style="margin-left: auto;">
                    <input type="text" class="ck-input ck-input-small" v-model="wip.hedef_kavite" :placeholder="__('Pin/Kavite No')" style="width: 100px; padding: 4px 8px; font-size: 12px; min-height: unset; height: 26px;" @click.prevent.stop />
                  </div>
                </label>
            ...
            <template v-else>
              <template v-if="selectedWipIds.length === 0">
