...
const props = defineProps<{
  show: boolean;
  wipId: string | null;
  nodes: any[]; // List of selectable nodes
}>();
...
function handleSubmit() {
  if (!selectedNodeId.value) {
    frappe.msgprint(__("Lütfen işlem yapılacak ucu seçin."));
    return;
  }
  emit("save", selectedNodeId.value);
}
</script>
    ...
    <div v-if="props.show" class="ck-modal-overlay">
      <div class="ck-modal ck-glass-modal" style="max-width: 400px;">
        <div class="ck-modal-header">
          <b>{{ __("İşlem Yapılacak Ucu Seçin") }}</b>
          ...
          <div class="ck-form-group">
            ...
            <div v-else class="node-list">
              <label v-for="node in props.nodes" :key="node.id" class="node-item" :class="{ 'is-selected': selectedNodeId === node.id }">
                ...
                <div class="node-info">
                  <div class="node-title">{{ node.display_title || node.type }}</div>
        ...
        <div class="ck-modal-footer">
          ...
          <button class="ck-btn ck-btn--primary" style="flex: 2" @click="handleSubmit">{{ __("Seç") }}</button>
        </div>
      </div>
    </div>
  </Teleport>
