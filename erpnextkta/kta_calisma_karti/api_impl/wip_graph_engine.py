...
def plug_into_socket(context):
    ...
    for wid in wip_ids:
        ...
        if wip.graph_state:
            ...
            if node_to_connect:
                # Statüyü Soketlendi olarak güncelle (artık merged_graph içinde)
                for n in merged_graph["nodes"]:
                    if n["id"] == node_to_connect["id"]:
                        n["status"] = "Soketlendi"
                
                # Hedef kaviteyi (pin_number) bu WIP'in tüketim kaydından okumalıyız
                # context içinde pin_number eskiden tek bir değerdi, artık dict veya hammadde kaydından gelecek.
                # Şimdilik hammadde kayıtlarından `hedef_kavite`'yi çekelim:
                pin_number = None
                hammadde_kaydi = frappe.get_all("KTA Calisma Karti Hammadde Kayitlari", 
                    filters={"parent": context.get("calisma_karti"), "alt_operasyon_ref": op_ref, "wip_id": wid},
                    fields=["hedef_kavite"]
                )
                if hammadde_kaydi and hammadde_kaydi[0].hedef_kavite:
                    pin_number = hammadde_kaydi[0].hedef_kavite
